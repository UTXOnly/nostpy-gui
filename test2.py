...
class Event:
    def __init__(self, relays, output_widget, treeview) -> None:
        self.relays = relays
            ...
            try:
                async with websockets.connect(relay) as ws:
                    ...
                    while (
                        ...
                        try:
                            response = await asyncio.wait_for(ws.recv(), timeout=1)
                            self.print_color(f"Response from {relay}:\n{response}, tye response {type(response)}", "32")
                            responses_received += 1
    ...
    def update_treeview(self, response_sting):
        try:
                        # Clear existing data in Treeview
            for item in self.treeview.get_children():
                self.treeview.delete(item)

            for item in response_sting:
                response = ast.literal_eval(item)
                self.print_color(f"{response[2]} is of datatype {type(response[2])}", "32")
                #data = ast.literal_eval(response[2])
                #data = json.loads(response[2])
                data = response[2]
                self.print_color(f"Received data: {data} of type {type(data)}", "32")  # Debug: Log received data
...
class DarkModeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Nostpy Admin GUI")
